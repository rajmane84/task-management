...
import { checkUsernameSchema, updateUserSchema } from "../zod-schemas/user.schema";
...
export const handleCheckUsernameExists = async (req: Request, res: Response) => {
  try {
    ...
    const userExists = await User.exists({ username: username.toLowerCase().trim() });
